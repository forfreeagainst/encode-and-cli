 ...
 * @param cwd
 * @param pkg
 * @returns luofeng-eslint-config/index
 * @returns luofeng-eslint-config/react
 * @returns luofeng-eslint-config/typescript/index
 * @returns luofeng-eslint-config/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    'luofeng-eslint-config/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}
